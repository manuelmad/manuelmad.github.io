...
function ocultarColgador() {
	...
	document.getElementById("segunda_fila_estallidoPP").innerHTML = "Superficie";
	document.getElementById("segunda_fila_estallidoAG").innerHTML = "Superficie";
...
function mostrarColgador() {
	...
	document.getElementById("segunda_fila_estallidoPP").innerHTML = "Colgador";
	document.getElementById("segunda_fila_estallidoAG").innerHTML = "Colgador";
...
// AÑADIR EL EVENTO DE BORRAR EL GRÁFICO AL BOTÓN CORRESPONDIENTE
...
// AÑADIR EL EVENTO DE MOSTRAR LÍNEAS DE DISEÑO Y REVESTIDOR AL BOTÓN DE HTML CORRESPONDIENTE
...
boton_tabla.addEventListener("click", calcularTablaEstPP);


// FUNCIÓN PARA REALIZAR EL CÁLCULO DE DISEÑO POR PRUEBA DE PRESIÓN
function calcularTablaEstPP()
{
	...
	var dens_frac = Number(document.getElementById("Dfrac_rev").value);
	var dens_lodo = Number(document.getElementById("DL_rev").value);

	var pres_fondo_intE = 0.052*dens_frac*prof_asent_tvd + 0.052*grad_fract_seg*prof_asent_tvd;
	var pres_colg_intE = pres_fondo_intE - (0.052*dens_lodo*(prof_asent_tvd-prof_colg_tvd));
	var pres_sup_intE = pres_fondo_intE - (0.052*dens_lodo*prof_asent_tvd);
	...
	var pres_poro_colg = 0.052*dens_poro_colg*prof_colg_tvd;

	var dens_poro = Number(document.getElementById("Dporo_rev").value);
	var pres_poro = 0.052*dens_poro*prof_asent_tvd;
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		var pres_R_fondoE = pres_fondo_intE- pres_poro;
	}
	else {
		var pres_R_supE = pres_colg_intE - pres_poro_colg;
		var pres_R_fondoE = pres_fondo_intE- pres_poro_colg;
	}


	// PERFIL DE PRESIÓN DE DISEÑO
	var pres_D_supE = pres_R_supE*FDestallidoSupInt;
	var pres_D_fondoE = pres_R_fondoE*FDestallidoSupInt;
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		document.getElementById("profSuperfTablaE").innerHTML = prof_colg_tvd;
		document.getElementById("segunda_fila_estallidoPP").innerHTML = "Colgador";
	}
...
// AÑADIR EL EVENTO DE CALCULAR SARTA AL BOTÓN DE HTML
var boton_sarta_perf = document.getElementById("calculos_sarta_perf");
boton_sarta_perf.addEventListener("click", calcularSartaPerf);

function calcularSartaPerf() {
	var diam_prox_hoyo = Number(document.getElementById("diam_prox_hoyo").value);
	var tipoDePerf = document.getElementById("tipoDePerf").value;
	var diam_portamecha = document.getElementById("diam_portamecha");
	var long_bha = document.getElementById("long_bha");

	var diam_tub_perf = document.getElementById("diam_tub_perf");

	// Código para los portamechas y BHA
	if(diam_prox_hoyo > 17.5) {
		diam_portamecha.innerHTML = 10;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 300;
		}
		else {
			long_bha.innerHTML = 100;
		}
	}

	else if(diam_prox_hoyo > 12.25 &&  diam_prox_hoyo <= 17.5) {
		diam_portamecha.innerHTML = 9.5;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 500;
		}
		else {
			long_bha.innerHTML = 300;
		}
	...
	else if(diam_prox_hoyo > 10.625 &&  diam_prox_hoyo <= 12.25) {
		diam_portamecha.innerHTML = 9;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 500;
		}
		else {
	...
	else if(diam_prox_hoyo > 8.75 &&  diam_prox_hoyo <= 10.625) {
		diam_portamecha.innerHTML = 8;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 600;
		}
		else {
			long_bha.innerHTML = 300;
		}
	...
	else if(diam_prox_hoyo > 7 &&  diam_prox_hoyo <= 8.75) {
		diam_portamecha.innerHTML = 6.75;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 600;
		}
		else {
	...
	else if(diam_prox_hoyo > 5.875 &&  diam_prox_hoyo <= 7) {
		diam_portamecha.innerHTML = 4.75;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 800;
		}
		else {
			long_bha.innerHTML = 400;
		}
	}

	else if(diam_prox_hoyo > 4.75 &&  diam_prox_hoyo <= 5.875) {
		diam_portamecha.innerHTML = 4.125;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 800;
		}
		else {
	...
	else if(diam_prox_hoyo >= 3.75 &&  diam_prox_hoyo <= 4.75) {
		diam_portamecha.innerHTML = 3.5;
		if(tipoDePerf == 1) {
			long_bha.innerHTML = 800;
		}
		else {
	...
	else if(diam_prox_hoyo < 3.75) {
		diam_portamecha.innerHTML = "No se utiliza portamechas";
		if(tipoDePerf == 1) {
			long_bha.innerHTML = "-";
		}
		else {
	...
	// Código para la tubería de perforación
	if(diam_prox_hoyo > 12.25) {
		diam_tub_perf.innerHTML = 5;
	}

	else if(diam_prox_hoyo > 7.625 &&  diam_prox_hoyo <= 12.25) {
		diam_tub_perf.innerHTML = 4.5;
	}

	else if(diam_prox_hoyo > 4.75 &&  diam_prox_hoyo <= 7.625) {
		diam_tub_perf.innerHTML = 3.5;
	}

	else if(diam_prox_hoyo > 2.875 &&  diam_prox_hoyo <= 4.75) {
		diam_tub_perf.innerHTML = 2.875;
	}

	else if(diam_prox_hoyo <= 2.875) {
		diam_tub_perf.innerHTML = 2.375;
	}
}


// AÑADIR EL EVENTO DE CALCULAR ALTURA DE GAS AL BOTÓN DE HTML
var boton_altura_gas = document.getElementById("calculo_altura_gas");
boton_altura_gas.addEventListener("click", calcularAlturaGas);


function calcularAlturaGas() {
	// Gradiente de Fractura de Seguridad 
	var tipo_de_pozo = document.getElementById("tipoDePozo").value;
	var grad_fract_seg;
	var vol_gas;

	if(tipo_de_pozo == 1) // Exploratorio
	{
		grad_fract_seg = 0.5;
		vol_gas = 561.46; // pie3
	}

	else if(tipo_de_pozo == 2) // Desarrollo
	{
		grad_fract_seg = 0.2;
		vol_gas = 280.73; // pie3
	}
	...
	var diam_prox_hoyo = Number(document.getElementById("diam_prox_hoyo").value);
	var diam_portamecha = Number(document.getElementById("diam_portamecha").innerHTML);
	var long_bha = Number(document.getElementById("long_bha").innerHTML);

	var vol_anular_alred_pm = (0.7854*(diam_prox_hoyo**2 - diam_portamecha**2)*long_bha)/144;

	var diam_tub_perf = Number(document.getElementById("diam_tub_perf").innerHTML);
	var prof_rev_md = Number(document.getElementById("prof_rev_md").value);
	var prof_rev_prox_md = Number(document.getElementById("prof_rev_prox_md").value);

	var long_tp = prof_rev_prox_md - long_bha - prof_rev_md;
	var vol_anular_alred_tp = (0.7854*(diam_prox_hoyo**2 - diam_tub_perf**2)*(long_tp))/144;
	
	var vol_anular_hoyoabierto = vol_anular_alred_pm + vol_anular_alred_tp;
	var altura_arrem_gas;

	var long_hoyo_abierto = prof_rev_prox_md - prof_rev_md;

	// Código condicional para calcular la altura de gas
	if(vol_gas <= vol_anular_alred_pm) {
		altura_arrem_gas = (144*vol_gas)/(0.7854*(diam_prox_hoyo**2 - diam_portamecha**2));
		console.log(vol_gas + " <= " + vol_anular_alred_pm);
		console.log("La altura de la arremetida de gas es " + altura_arrem_gas + " pies.");
		document.getElementById("altura_gas_md").innerHTML = altura_arrem_gas.toFixed(1);

		var tope_gas_md = prof_rev_prox_md - altura_arrem_gas;
		document.getElementById("tope_gas_md").innerHTML = tope_gas_md.toFixed(1);
		console.log("El tope de la arremetida de gas es: " + tope_gas_md + " pies.");

		document.getElementById("container-aviso-alturagas").style.display = "none";
	}
	else if(vol_anular_hoyoabierto >= vol_gas && vol_gas > vol_anular_alred_pm) {
		altura_arrem_gas = long_bha + (144*(vol_gas - vol_anular_alred_pm)/(0.7854*(diam_prox_hoyo**2 - diam_tub_perf**2)));
		console.log(vol_anular_hoyoabierto + " >= " + vol_gas + " y " + vol_gas + " > " + vol_anular_alred_pm);
		console.log("La altura de la arremetida de gas es " + altura_arrem_gas + " pies.");
		document.getElementById("altura_gas_md").innerHTML = altura_arrem_gas.toFixed(1);
	...
	else if(vol_gas > vol_anular_hoyoabierto) {
		document.getElementById("container-aviso-alturagas").style.display = "block";
		console.log(vol_gas + " > " + vol_anular_hoyoabierto);
		console.log(`La altura de gas es mayor a la longitud del próximo hoyo abierto (${long_hoyo_abierto} pies). Debe estimar el diámetro interno del revestidor analizado para calcular la altura exacta de la arremetida de gas.`);
		document.getElementById("aviso_altura_gas").innerHTML = `La altura de gas es mayor a la longitud del próximo hoyo abierto (${long_hoyo_abierto} pies). Debe estimar el diámetro interno del revestidor analizado para calcular la altura exacta de la arremetida de gas.`;
	}

	var datos = [long_bha, long_tp, vol_gas, vol_anular_alred_pm, vol_anular_alred_tp, diam_tub_perf, prof_rev_prox_md];
	return datos;
}

// AÑADIR EL EVENTO DE CALCULAR ALTURA DE GAS 2 AL BOTÓN DE HTML
var boton_altura_gas2 = document.getElementById("calculo_altura_gas2");
boton_altura_gas2.addEventListener("click", calcularAlturaGas2);

function calcularAlturaGas2() {
	var diam_int_estimado_rev = Number(document.getElementById("diam_int_estimado_rev").value);
	var datos_previos = calcularAlturaGas();

	var altura_arrem_gas = datos_previos[0] + datos_previos[1] + (144*(datos_previos[2] - datos_previos[3] - datos_previos[4])/(0.7854*(diam_int_estimado_rev**2 - datos_previos[5]**2)));
	document.getElementById("altura_gas_md").innerHTML = altura_arrem_gas.toFixed(1);
	console.log("La altura de la arremetida de gas es " + altura_arrem_gas + " pies.");

	var tope_gas_md = datos_previos[6] - altura_arrem_gas;
	document.getElementById("tope_gas_md").innerHTML = tope_gas_md.toFixed(1);
	console.log("El tope de la arremetida de gas es: " + tope_gas_md + " pies.")
}

// AÑADIR EL EVENTO DE CALCULAR TABLA DE DISEÑO AL BOTÓN DE HTML
var boton_tabla_AG = document.getElementById("calculo_tabla_diseno_AG");
boton_tabla_AG.addEventListener("click", calcularTablaEstAG);

// FUNCIÓN PARA REALIZAR EL CÁLCULO DE DISEÑO POR ARREMETIDA DE GAS
function calcularTablaEstAG()
{
	// b) Máxima carga de estallido: Arremetida de gas

	// Gradiente del gas de la arremetida
	var tipo_de_pozo = document.getElementById("tipoDePozo").value;
	var grad_fract_seg;
	var grad_gas_arrem;
	...
	console.log(grad_fract_seg);
	console.log(grad_gas_arrem);

	// Variables pendientes del proceso anterior
	var prof_rev_prox_tvd = Number(document.getElementById("prof_rev_prox_tvd").value);
	var tope_gas_tvd = Number(document.getElementById("tope_gas_tvd").value);
	var altura_gas_tvd = prof_rev_prox_tvd - tope_gas_tvd;

	document.getElementById("altura_gas_tvd").innerHTML = altura_gas_tvd.toFixed(1);

	var dens_prox_hoyo = Number(document.getElementById("dens_prox_hoyo").value);

	var presion_gas = 0.052*(dens_prox_hoyo + grad_gas_arrem)*prof_rev_prox_tvd;
	console.log("Presión de gas: " + presion_gas);

	var prof_asent_tvd = Number(document.getElementById("prof_rev_tvd").value);

	// Presión actuante en zapata
	var presion_act_zap;
	if(tope_gas_tvd > prof_asent_tvd) {
		presion_act_zap = presion_gas - (0.052*dens_prox_hoyo*(prof_rev_prox_tvd - altura_gas_tvd - prof_asent_tvd));
	}
	else {
		presion_act_zap = presion_gas;
	}
	console.log("Presión actuante en zapata: " + presion_act_zap);

	// Presión de fractura de seguridad en la zapata
	var dens_frac = Number(document.getElementById("Dfrac_rev").value);
	var pres_frac_zap = 0.052*dens_frac*prof_asent_tvd;
	var pres_frac_seg_zap = pres_frac_zap + 0.052*grad_fract_seg*prof_asent_tvd;

	console.log("Presión de fractura de seguridad en zapata: " + pres_frac_seg_zap);
	...
	if(presion_act_zap < pres_frac_seg_zap) {
		pres_zap_intE2 = presion_act_zap;
		if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
			pres_sup_intE2 = presion_gas - (0.052*dens_prox_hoyo*tope_gas_tvd);
		}
		else {
			pres_sup_intE2 = presion_gas - (0.052*dens_prox_hoyo*(tope_gas_tvd - prof_colg_tvd));
		}
	}
	else {
		pres_zap_intE2 = pres_frac_seg_zap;
		if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
			pres_sup_intE2 = pres_zap_intE2 - (0.052*dens_prox_hoyo*prof_asent_tvd);
		}
		else {
			pres_sup_intE2 = pres_zap_intE2 - (0.052*dens_prox_hoyo*(prof_asent_tvd - prof_colg_tvd));
		}
	}
	console.log("Presión en zapata: " + pres_zap_intE2);
	console.log("Presión en superficie: " + pres_sup_intE2);

	// PERFIL DE PRESIÓN EXTERNA //
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		var pres_sup_extE2 = 0;
	}
	else {
		var dens_poro_colg = Number(document.getElementById("Dporo_colg").value);
		var pres_poro_colg = 0.052*dens_poro_colg*prof_colg_tvd;
		var pres_sup_extE2 = pres_poro_colg;
	}
	...
	// PERFIL DE PRESIÓN RESULTANTE (Pint - Pext) //
	var pres_R_supE2 = pres_sup_intE2 - pres_sup_extE2;
	var pres_R_fondoE2 = pres_zap_intE2 - pres_poro;


	// PERFIL DE PRESIÓN DE DISEÑO //
	var pres_D_supE2 = pres_R_supE2*FDestallidoSupInt;
	var pres_D_fondoE2 = pres_R_fondoE2*FDestallidoSupInt;
	...
	document.getElementById("presionSuperfTablaE2").innerHTML = pres_D_supE2.toFixed(1);

	document.getElementById("profFondoTablaE2").innerHTML = prof_asent_tvd;
	document.getElementById("presionFondoTablaE2").innerHTML = pres_D_fondoE2.toFixed(1);
}
...
function disenoRevestidorEstallido()
{
	...
	// EJE Y
	for(a = delta_valores_prof/*50*/; a <= delta_prof; a = a+delta_valores_prof) // Línea cada 50 pies
	{
	...
	{
		for(a = delta_valores_prof/*500*/; a <= delta_prof; a = a+delta_valores_prof) // Guión cada 50 pies
		{
			if(a % 1000 == 0)
			{
	...
	if(max_presion <= 1000)
	{
		for(a = 0/*100*/; a <= max_presion; a = a+100)
		{
	...
	{
		for(a = 0/*500*/; a <= max_presion; a = a+500)
		{
			papel.fillText(a, (a*factorX)+margen, margen-10);
		}
	}
	...
	{
		for(a = 0/*1000*/; a <= max_presion; a = a+1000)
		{
			papel.fillText(a, (a*factorX)+margen, margen-10);
		}
	}
	...
	{
		for(a = 0/*2000*/; a <= max_presion; a = a+2000)
		{
			papel.fillText(a, (a*factorX)+margen, margen-10);
		}
	}
	
	// GRAFICAR LÍNEA DE DISEÑO POR PRUEBA DE PRESIÓN
	...
	// GRAFICAR LÍNEA DE DISEÑO POR ARREMETIDA DE GAS
	var presionSuperfTablaE2 = document.getElementById("presionSuperfTablaE2");
	var pres_D_supE2= presionSuperfTablaE2.innerHTML;

	var profSuperfTablaE2 = document.getElementById("profSuperfTablaE2");
	var prof_sup2 = profSuperfTablaE2.innerHTML;

	var presionFondoTablaE2 = document.getElementById("presionFondoTablaE2");
	var pres_D_fondoE2 = presionFondoTablaE2.innerHTML;

	var profFondoTablaE2 = document.getElementById("profFondoTablaE2");
	var prof_asent_tvd2 = profFondoTablaE2.innerHTML;

	dibujarLineaPapel("purple", (pres_D_supE2 *factorX)+margen, ((prof_sup2 - min_prof)* factorY)+margen, (pres_D_fondoE2* factorX)+margen, ((prof_asent_tvd2 - min_prof)* factorY)+margen);
...
function calcularTablaCol() {
	...
	var prof_rev_prox_tvd = Number(document.getElementById("prof_rev_prox_tvd").value);
	var prof_tope_lodo = prof_rev_prox_tvd / 3;
	var dens_prox_hoyo = Number(document.getElementById("dens_prox_hoyo").value);
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		var prof_sup = prof_colg_tvd;
	}
	var prof_ttc_tvd = Number(document.getElementById("ttc_tvd").value);
	var prof_asent_tvd = Number(document.getElementById("prof_rev_tvd").value);
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		if(prof_tope_lodo >= prof_colg_tvd) {
			var pres_sup_intC = 0;
		}
		else {
			var pres_sup_intC = 0.052*dens_prox_hoyo*(prof_colg_tvd - prof_tope_lodo);
		}
	}
	
	var pres_ttc_intC;
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		if(prof_tope_lodo >= prof_ttc_tvd) {
			pres_ttc_intC = 0;
		}
		else {
			pres_ttc_intC = 0.052*dens_prox_hoyo*(prof_ttc_tvd - prof_tope_lodo);
		}
	}
	else {
		if(prof_tope_lodo >= prof_colg_tvd) {
			...
			var pres_ttc_intC = 0.052*dens_prox_hoyo*(prof_colg_tvd - prof_tope_lodo);
		}
	}

	var pres_fondo_intC;
	if(prof_tope_lodo >= prof_asent_tvd) {
		pres_fondo_intC = 0;
	}
	else {
		pres_fondo_intC = 0.052*dens_prox_hoyo*(prof_asent_tvd - prof_tope_lodo);
	}
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		var pres_sup_extC = 0.052*dens_lodo*prof_colg_tvd;
	}
	
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		var pres_ttc_extC = 0.052*dens_lodo*prof_ttc_tvd;
	}
	else {
		var pres_ttc_extC = 0.052*dens_lodo*prof_colg_tvd;
	}
	
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		var pres_fondo_extC = pres_ttc_extC + 0.052*den_lech_llen*(interf_lech_tvd - prof_ttc_tvd) + 0.052*den_lech_cola*(prof_asent_tvd - interf_lech_tvd);
	}
	else {
		var pres_fondo_extC = pres_ttc_extC + 0.052*den_cem*(prof_asent_tvd - prof_colg_tvd);
	}
	

	// PERFIL DE PRESIÓN RESULTANTE (Pext - Pint)
	var pres_R_supC = pres_sup_extC - pres_sup_intC;
	var pres_R_ttcC = pres_ttc_extC - pres_ttc_intC;
	var pres_R_fondoC = pres_fondo_extC - pres_fondo_intC;


	// PERFIL DE PRESIÓN DE DISEÑO
	var pres_D_supC = pres_R_supC*FDcolapsoSupInt;
	var pres_D_ttcC = pres_R_ttcC*FDcolapsoSupInt
	var pres_D_fondoC = pres_R_fondoC*FDcolapsoSupInt;
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none") {
		...
		if(prof_ttc_tvd !=0) {
			...
			document.getElementById("profTTCTablaC").innerHTML = "N/A";
...
function disenoRevestidorColapso()
{
	...
	// GRAFICAR LÍNEA DE DISEÑO POR 1/3 VACÍO
	...
	if(prof_sup != "N/A" && prof_ttc_tvd != "N/A") {
		dibujarLineaLienzo("blue", (pres_D_supC *factorX)+margen, ((prof_sup - min_prof)* factorY)+margen, (pres_D_ttcC *factorX)+margen, ((prof_ttc_tvd - min_prof)* factorY)+margen);
	...
	else if(prof_sup != "N/A" && prof_ttc_tvd == "N/A") {
	...
	if(prof_sup != "N/A" && prof_ttc_tvd != "N/A") {
		dibujarLineaSeleccionLienzo("green", (resistencia_colapso_rev*factorX)+margen, ((prof_sup - min_prof)*factorY)+margen, (resistencia_colapso_rev*factorX)+margen, ((prof_ttc_tvd - min_prof) *factorY)+margen);
...
boton5.addEventListener("click", disenoTension);


// ANÁLISIS REVESTIDOR POR TENSIÓN //
function disenoTension()
{
	// CÁCULO DE DISEÑO //
	...
	if(W_TTC != "N/A" && prof_ttc_md !=0)
	{
	...
	else if(W_TTC != "N/A" && prof_ttc_md == 0) {
		...
		document.getElementById("tensionFactorTTCTablaT").innerHTML = "N/A";
	}
	else if(W_TTC == "N/A")
	{
	...
	//document.getElementById("tensionFactorFondoTablaT").innerHTML = (fuerza_axial_fondo*FDtensionProd).toFixed(1);
	...
	if(condicion_sup != "N/A") {
		document.getElementById("profSuperfTablaEC").innerHTML = prof_sup;
	}
	else {
		document.getElementById("profSuperfTablaEC").innerHTML = prof_colg_tvd;
	}
	document.getElementById("profFondoTablaEC").innerHTML = prof_rev_tvd;
...
function graficaTensionyBiaxial()
{
	...
	var profTTCTablaT = document.getElementById("profTTCTablaT").innerHTML;
	var prof_ttc = Number(profTTCTablaT);
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none")
	{
		if(profTTCTablaT != "N/A") {
			...
			dibujarLineaHoja("black", (anchoT-((max_tension- fuerza_axial_sup/1000)*factorXT)) + margenT, (prof_sup* factorYT) + margenT, (anchoT-((max_tension- fuerza_axial_fondo/1000)*factorXT)) + margenT, (prof_rev_md* factorYT) + margenT);
		}
	}
	else
	{
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none")
	{
		if(profTTCTablaT != "N/A") {
			...
			dibujarLineaHoja("red", (anchoT-((max_tension- (fuerza_axial_sup/1000 + overpull/1000))*factorXT)) + margenT, (prof_sup* factorYT) + margenT, (anchoT-((max_tension- (fuerza_axial_fondo/1000 + overpull/1000))*factorXT)) + margenT, (prof_rev_md* factorYT) + margenT);
		}
	...
	if(prof_colg_tvd == 0 || parrafo_colgador.style.display == "none")
	{
		if(profTTCTablaT != "N/A") {
			...
			dibujarLineaHoja("purple", (anchoT-((max_tension- (tension_factor_sup/1000))*factorXT)) + margenT, (prof_sup* factorYT) + margenT, (anchoT-((max_tension- (0/1000))*factorXT)) + margenT, (punto_neutro* factorYT) + margenT);
		}
...
function calculoFactores()
{
	...
	if(FDE_S < FDestallidoSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN SUPERFICIE //
	{
	...
	if(FDE_F < FDestallidoSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN FONDO //
	{
	...
	if(FDC_S < FDcolapsoSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN SUPERFICIE //
	{
	...
	if(FDC_F < FDcolapsoSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN FONDO //
	{
	...
	if(FDT_S < FDtensionSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN SUPERFICIE //
	{
	...
	if(FDT_F < FDtensionSupInt) // DENOTAR QUE EL REVESTIDOR FALLA EN FONDO //
	{
