...
{ 
   window.open(URL,"INSTRUCCIONES","width=800,height=400,scrollbars=YES");
}
...
dibujarLinea("blue", 80, 700, 130, 700);
lienzo.font = '10px "Tahoma"';
lienzo.fillStyle = "black";
lienzo.fillText("Densidad de Poro", 135, 703);

dibujarLinea("green", 230, 700, 280, 700);
lienzo.font = '10px "Tahoma"';
lienzo.fillStyle = "black";
lienzo.fillText("Densidad de Lodo", 285, 703);

dibujarLinea("orange", 380, 700, 430, 700);
lienzo.font = '10px "Tahoma"';
lienzo.fillStyle = "black";
lienzo.fillText("Dens. de M. de Arrem.", 435, 703);

dibujarLinea("red", 550, 700, 600, 700);
lienzo.font = '10px "Tahoma"';
lienzo.fillStyle = "black";
lienzo.fillText("Densidad de Fractura", 605, 703);
...
var x = 20; // COORDENADA X DONDE ESTARÁ EL TEXTO ROTADO //
var y = 380; // COORDENADA Y DONDE ESTARÁ EL TEXTO ROTADO //
...
var x = 380; // COORDENADA X DONDE ESTARÁ EL TEXTO //
var y = 40; // COORDENADA Y DONDE ESTARÁ EL TEXTO //
lienzo.save(); // GRABAR EL CANVAS PARA HACER LOS SIGUIENTES CAMBIOS Y QUE NO AFECTEN AL CANVAS ORIGINAL //
lienzo.translate(x,y); // TRASLADAR EL PUNTO DE DIBUJO DEL CANVAS A LAS COORDENADAS ANTES ESTABLECIDAS //
lienzo.textAlign = 'center'; // ESTABLECER ALINEACIÓN DEL TEXTO //
lienzo.font = '16px "Tahoma"'; // ESTABLECER FUENTE Y TAMAÑO DEL TEXTO //
...
function trazarCurva()
{
	...
	{
		lienzo.fillText(a, 35, a*factorY+3 + margen);
	}
	...
	{
		lienzo.fillText(a, a*factorX-4 + margen, 65);
	}
	lienzo.fillText(0, 70, 70); // Origen de coordenadas //
	lienzo.fillStyle = "purple"; // VALOR DE COLOR PÚRPURA //
	lienzo.fillText(9, 9*factorX-4 + margen, 65); // Valor de 9 lpg (Presión Normal) //
...
function autoLineas()
{
	...
	while(x >= P_vs_DA[0])
	{
		...
		for(n=0; n<=datosDA-1; n=n+2)
		{
			if(x >= P_vs_DA[n] && x< P_vs_DA[n+2])
			{
				...
				{
					lienzo.fillText(mostrar_y + "'", 700, auto1 + margen); // Mostrar en canvas los valores de profundidad calculados
				}
			}
		}
	...
	lienzo.fillText(ultimaProf/factorY +"'", 700, ultimaProf + margen); // Mostrar valor de profundidad final en canvas //
...
function lineaDiseno()
{
	...
	if(mostrar_y1 >= 50)
	{
		lienzo.fillText(mostrar_y1, 700, y1 + margen);
	}
	
	var mostrar_y2 = (y2/factorY).toFixed(0);
	if(mostrar_y2 >= 50)
	{
		lienzo.fillText(mostrar_y2, 700, y2 + margen);
	}
}
...
function lineaDiseno3()
{
	...
	if(y2 != y5) // como y2 = y5, debo especificar que no se muestre 2 veces el valor
	{
		...
		if(mostrar_y5 >= 50)
		{
			lienzo.fillText(mostrar_y5, 700, y5 + margen);
		}
	}
	...
	if(mostrar_y6 >= 50)
	{
		lienzo.fillText(mostrar_y6, 700, y6 + margen);
	}
}
...
function lineaDiseno5()
{
	...
	if(y6 != y9) // como y6 = y9, debo especificar que no se muestre 2 veces el valor
	{
		...
		if(mostrar_y9 >= 50)
		{
			lienzo.fillText(mostrar_y9, 700, y9 + margen);
		}
	}
	...
	if(mostrar_y10 >= 50)
	{
		lienzo.fillText(mostrar_y10, 700, y10 + margen);
	}
}
...
function lineaDiseno7()
{
	...
	if(y10 != y13)
	{
		...
		if(mostrar_y13 >= 50)
		{
			lienzo.fillText(mostrar_y13, 700, y13 + margen);
		}
	}
	...
	if(mostrar_y14 >= 50)
	{
		lienzo.fillText(mostrar_y14, 700, y14 + margen);
	}	
}
	...
	if(Prof_Normal_Anormal != "" && Prof_Rev_Int != "" && Densidad_Zap_Int != "")
	{
		...
		else
		{
			...
			for(m=0; m<=filas_cuerpotabla-1; m=m+1)
			{
				...
				var celda1_prof2 = celda1_dl.nextElementSibling.nextElementSibling;
				var Prof1_2 = celda1_prof2.firstElementChild;
				var Prof12 = Number(Prof1_2.value);

				var celda1_pfrac = celda1_prof2.nextElementSibling.nextElementSibling;
				var PreF_1 = celda1_pfrac.firstElementChild;
				var PreF1 = Number(PreF_1.value);
